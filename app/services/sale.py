    ...
    async def create_sale(self, data: SalesTransactionCreate) -> SalesTransactionRead:
        ...
        for item in data.items:
            ...
            if not product.is_active:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"El producto '{product.name}' no está disponible para la venta")

            price = await self.price_repo.get_current(item.product_id)
            if not price:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"El producto '{product.name}' no tiene precio registrado")
        ...
        for item_data in resolved_items:
            ...
            await self.stock_repo.apply_delta(item_data["product_id"], -item_data["quantity"])

        await self.session.commit()
        return await self.get_by_id(transaction.id)

    async def update_status(self, id: UUID, data: SalesTransactionStatusUpdate) -> SalesTransactionRead:
        transaction = await self.tx_repo.get_by_id(id)
        if not transaction:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Transacción no encontrada")
        if transaction.status == TransactionStatus.CANCELLED:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="La transacción ya está cancelada")
